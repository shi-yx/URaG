...
from training.data_retrieval import make_supervised_data_module
from training.params import DataArguments, ModelArguments, TrainingArguments
from training.train_utils import (get_peft_state_maybe_zero_3,
...
def train():
    ...
    if training_args.lora_enable:
        ...
        lora_list = find_target_linear_names(model, lora_namespan_exclude=lora_namespan_exclude, num_lora_modules=training_args.num_lora_modules)
        lora_list_ = [lora_module for lora_module in lora_list if ('model.layers.' not in lora_module)]
        print(f"lora_list_:{lora_list_}!!!!!!!!!!!")

        peft_config = LoraConfig(
            r=training_args.lora_rank,
            lora_alpha=training_args.lora_alpha,
            target_modules=lora_list_,
    ...
    if training_args.lora_enable:
        proj_layer_params = model.proj_layer.parameters()
        set_requires_grad(proj_layer_params, True)
